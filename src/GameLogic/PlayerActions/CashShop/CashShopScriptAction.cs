...
public class CashShopScriptAction
{
    ...
    public async ValueTask SetScriptAsync(Player player)
    {
        await player.InvokeViewPlugInAsync<ICashShopScriptPlugIn>(p => p.CashShopScriptAsync(512, 2012, 084)).ConfigureAwait(false);
    }
}